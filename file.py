...
def generate_signal_from_closes(closes, fast, slow, min_diff_pct=0.2):
    ...
    min_diff_pct (float): minimum % distance between fast & slow SMA
                          before a signal is considered valid.
    """

    if len(closes) < slow:
        return "HOLD"

    f_curr = simple_sma(closes, fast)
    s_curr = simple_sma(closes, slow)
...
def update_dynamic_levels(state, current_price, tp_step=0.8, sl_step=0.8):
    """
...
def run_cycle(cfg, state):
    ...
    for symbol in symbols:
        try:
            ...
            sig = generate_signal_from_closes(closes, fast, slow, min_diff_pct=min_diff_pct)
            last_price = closes[-1]
            ...
            if sig == "BUY" and pos is None:
                ...
                if last_price > entry:
                    new_tp = last_price*(1+tp_pct)
                    if new_tp > pos["tp"]:
                        logging.info(f"{symbol}: Raising TP from {pos['tp']:.6f} -> {new_tp:.6f}")
                        pos["tp"] = new_tp
                    new_sl = last_price * (1 - sl_pct)
                    if new_sl > pos["sl"]:
                        logging.info(f"{symbol}: Raising SL from {pos['sl']:.6f} -> {new_sl:.6f}")
                        pos["sl"] = max(new_sl, entry)
                # --- ATR-based dynamic TP/SL (fallback to percent config if ATR unavailable) ---
                atr = calculate_atr(kl, period=14)
                if atr is not None:
                    tp_level = entry + (atr * 1.0)   # 1 ATR profit target (tight)
                    sl_level = entry - (atr * 0.5)   # 0.5 ATR stop (tight)
                else:
